...
namespace EDDiscovery
{
    public partial class EDDiscoveryForm
    {
        ...
        private string DLLGetScanData(string systemname, bool edsmlookup)
        {
            if (systemname.IsEmpty())
                systemname = history.CurrentSystem()?.Name;

            if (systemname.HasChars())
            {
                var sc = history.StarScan;
                var snode = sc.FindSystemSynchronous(new SystemClass(systemname), edsmlookup);
                var json = snode != null ? JToken.FromObject(snode, true, new Type[] { typeof(System.Drawing.Image) }, 12, System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public) : null;
        ...
        private string DLLGetVisitedList()
        {
            var vlist = history.Visited.Values.Select(x => new Tuple<string,long,double,double,double>(x.System.Name,x.System.SystemAddress??-1,x.System.X,x.System.Y,x.System.Z)).ToList();
        ...
        public Tuple<string, string, string, string> DLLStart(ref string alloweddlls)
        {
            ...
            string[] options = new string[] { EDDDLLInterfaces.EDDDLLIF.FLAG_HOSTNAME + "EDDiscovery",
                                              ...
                                              EDDDLLInterfaces.EDDDLLIF.FLAG_CALLVERSION + EDDDLLInterfaces.EDDDLLIF.CallBackVersion.ToStringInvariant(),
                                            };
