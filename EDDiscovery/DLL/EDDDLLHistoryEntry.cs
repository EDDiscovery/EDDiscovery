    ...
    static public class EDDDLLCallerHE
    {
        static public EDDDLLIF.JournalEntry CreateFromHistoryEntry(EliteDangerousCore.HistoryEntry he)
        {
            ...
            else
            {
                ...
                je.materials = (from x in he.MaterialCommodity.Sort(false) select x.Name + ":" + x.Count.ToStringInvariant() + ":" + x.FDName).ToArray();
                je.commodities = (from x in he.MaterialCommodity.Sort(true) select x.Name + ":" + x.Count.ToStringInvariant() + ":" + x.FDName).ToArray();
