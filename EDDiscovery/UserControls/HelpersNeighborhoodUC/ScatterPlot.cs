...
namespace EDDiscovery.UserControls
{
    public partial class ScatterPlot : UserControl
    {
        ...
        Color[] colorIdx = new Color[] { Color.Blue, Color.AliceBlue, Color.Yellow, Color.Orange, Color.OrangeRed, Color.Red, Color.Fuchsia, Color.Aqua, Color.Black };
        ...
        public void AddPoint(double x, double y, double z, int series)
        {
            ...
            foreach (List<double[]> ser in Points)
            {
                if (ProjPoints.Count - 1 < series)
                    ProjPoints.Add(ScatterProjection.ProjectVector(ser, this.Width, this.Height, f, d_w, azimuth, elevation));
                else
                    ProjPoints[series] = ScatterProjection.ProjectVector(ser, this.Width, this.Height, f, d_w, azimuth, elevation);
            }
            this.Invalidate();
        }
