...
namespace EDDiscovery.UserControls
{
    class Projection
    {
        ...
        {
            Algebra.Matrix<double> Mext = GetMext(azimuth, elevation, d_w);
            Algebra.Matrix<double> Mint = GetMint(s_x, s_y, f);
            Algebra.Matrix<double> X_h = new Algebra.Matrix<double>(4, 1);
            X_h.SetMatrix(new double[] { x[0], x[1], x[2], 1.0 });
            //Debug.Print((Mint * Mext).ToString());
            Algebra.Matrix<double> P = Mint * Mext * X_h;
        ...
        static Algebra.Matrix<double> GetMint(double s_x, double s_y, double f)
        {
            Algebra.Matrix<double> Mint = new Algebra.Matrix<double>(3, 3);
            double o_x = s_x / 2;
            double o_y = s_y / 2;
            double a = 1;
        ...
        static Algebra.Matrix<double> GetMext(double azimuth, double elevation, double[] d_w)
        {
            Algebra.Matrix<double> R = RotationMatrix(azimuth, elevation);
            Algebra.Matrix<double> dw = new Algebra.Matrix<double>(3, 1);
            dw.SetMatrix(d_w);
            Algebra.Matrix<double> Mext = R | (-R * dw);
            return Mext;
        }

        static Algebra.Matrix<double> RotationMatrix(double azimuth, double elevation)
        {
            Algebra.Matrix<double> R = new Algebra.Matrix<double>(3, 3);
