...
{
    public partial class UserControlStarsInRange : UserControlCommonBase
    {
        private StarDistanceInRange computer;

        public UserControlStarsInRange()
        {
            InitializeComponent();
            var corner = dataGridViewStarsInRange.TopLeftHeaderCell; // work around #1487
        }

        public override void Init()
        {
            computer = new StarDistanceInRange();
        ...
        {
            SetControlText("");
            dataGridViewStarsInRange.Rows.Clear();
            ...
            if (csl.Count() > 0)
            {
                ...
                foreach (KeyValuePair<double, ISystem> tvp in csl)
                {
                    ...
                    { 
                        int rowindex = dataGridViewStarsInRange.Rows.Add(rowobj);
                        dataGridViewStarsInRange.Rows[rowindex].Tag = tvp.Value;
                    }
                }
            }
        ...
        private void dataGridViewNearest_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (dataGridViewStarsInRange.SelectedCells.Count < 2 || dataGridViewStarsInRange.SelectedRows.Count == 1)      // if single row completely selected, or 1 cell or less..
            {
                DataGridView.HitTestInfo hti = dataGridViewStarsInRange.HitTest(e.X, e.Y);
                if (hti.Type == DataGridViewHitTestType.Cell)
                {
                    dataGridViewStarsInRange.ClearSelection();                // select row under cursor.
                    dataGridViewStarsInRange.Rows[hti.RowIndex].Selected = true;

                    if (e.Button == MouseButtons.Right)         // right click on travel map, get in before the context menu
                    {
                        rightclickrow = hti.RowIndex;
                        rightclicksystem = (ISystem)dataGridViewStarsInRange.Rows[hti.RowIndex].Tag;
                    }
                }
            }
        ...
        {
            IEnumerable<DataGridViewRow> selectedRows = dataGridViewStarsInRange.SelectedCells.Cast<DataGridViewCell>()
