...
namespace EDDiscovery.UserControls
{
    public partial class UserControlModules : UserControlCommonBase
    {
        ...
        private void DisplayShip(Ship si, bool displaydeleteloadoutbutton)
        {
            ...
            extPanelRollUpStats.Visible = !ItemData.IsSRVOrFighterOrLander(si.ShipFD);
        ...
        private void buttonExtCoriolis_Click(object sender, EventArgs e)
        {
            ...
            {
                string errstr;
                string coriolis = last_si.JSONCoriolis(out errstr).ToString();
            ...
            if (dataGridViewModules.RowCount > 0)
            {
                ...
                if (last_si != null)
                {
                    frm.ShowOptionalButton("Loadout", () =>
                     {
                         frm.Close();
                         string loadoutjournalline = last_si.JSONLoadout(true).ToString();
                         ExtendedControls.InfoForm info = new ExtendedControls.InfoForm();
                         info.Info("Loadout", this.FindForm().Icon, loadoutjournalline);
                         info.Show(this);
                     });
                }
