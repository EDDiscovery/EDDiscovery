...
namespace EDDiscovery.DB
{
    ...
    public class SystemClassDB : EliteDangerous.SystemClass
    {
        ...
        {
            system = new EliteDangerous.SystemClass
            {
                name = refsys.name,
        ...
        private static Dictionary<long, EliteDangerous.SystemClassBase> GetEdsmSystemsLite()
        {
            using (SQLiteConnectionSystem cn = new SQLiteConnectionSystem(mode: EDDbAccessMode.Reader))
            {
                Dictionary<long, EliteDangerous.SystemClassBase> systemsByEdsmId = new Dictionary<long, EliteDangerous.SystemClassBase>();
                ...
                using (DbCommand cmd = cn.CreateCommand("SELECT s.id, s.EdsmId, n.Name, s.x, s.y, s.z, s.UpdateTimestamp, s.gridid, s.randomid FROM EdsmSystems s JOIN SystemNames n ON n.EdsmId = s.EdsmId"))
                {
                    using (DbDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            EliteDangerous.SystemClassBase sys = new EliteDangerous.SystemClassBase
                            {
                                id = (long)reader["id"],
        ...
        private static long DoParseEDSMUpdateSystemsReader(JsonTextReader jr, ref string date, ref bool outoforder, Func<bool> cancelRequested, Action<int, string> reportProgress, bool useCache = true, bool useTempSystems = false)
        {
            ...
            Dictionary<long, EliteDangerous.SystemClassBase> systemsByEdsmId = useCache ? GetEdsmSystemsLite() : new Dictionary<long, EliteDangerous.SystemClassBase>();
            int count = 0;
            int updatecount = 0;
            int insertcount = 0;
                            ...
                            try
                            {
                                ...
                                while (!cancelRequested() && !SQLiteConnectionSystem.IsReadWaiting)
                                {
                                    ...
                                    if (coords != null && (coords["x"].Type == JTokenType.Float || coords["x"].Type == JTokenType.Integer))
                                    {
                                        ...
                                        EliteDangerous.SystemClassBase dbsys = null;
                                        ...
                                        if (!useTempSystems)
                                        {
                                            ...
                                            if (!useCache)
                                            {
                                                ...
                                                using (DbDataReader reader = selectSysCmd.ExecuteReader())
                                                {
                                                    if (reader.Read())
                                                    {
                                                        dbsys = new EliteDangerous.SystemClassBase
                                                        {
                                                            id = (long)reader["id"],
                                                            id_edsm = edsmid
                                                        };
        ...
        public static bool PerformEDSMFullSync(Func<bool> cancelRequested, Action<int, string> reportProgress, Action<string> logLine, Action<string> logError)
        {
            ...
            SystemClassDB.RemoveHiddenSystems();
            ...
            bool success = EDDiscovery.HTTP.DownloadFileHandler.DownloadFile(EDSMClass.ServerAddress + "dump/systemsWithCoordinates.json", edsmsystems, out newfile, (n, s) =>
            {
                ...
                using (var reader = new StreamReader(s))
                    updates = SystemClassDB.ParseEDSMUpdateSystemsStream(reader, ref rwsysfiletime, ref outoforder, true, cancelRequested, reportProgress, useCache: false, useTempSystems: true);
        ...
        public static void PerformEDDBFullSync(Func<bool> cancelRequested, Action<int, string> reportProgress, Action<string> logLine, Action<string> logError)
        {
            ...
            if (success)
            {
                ...
                long number = SystemClassDB.ParseEDDBUpdateSystems(systemFileName, logError);
        ...
        public static void PerformSync(Func<bool> cancelRequested, Action<int, string> reportProgress, Action<string> logLine, Action<string> logError, SystemsSyncState state)           // big check.. done in a thread.
        {
            ...
            if (state.performedsmsync || state.performeddbsync)
            {
                if (state.performedsmsync && !cancelRequested())
                {
                    ...
                    try
                    {
                        state.performhistoryrefresh |= SystemClassDB.PerformEDSMFullSync(cancelRequested, reportProgress, logLine, logError);
                ...
                if (!cancelRequested())
                {
                    ...
                    try
                    {
                        SystemClassDB.PerformEDDBFullSync(cancelRequested, reportProgress, logLine, logError);
