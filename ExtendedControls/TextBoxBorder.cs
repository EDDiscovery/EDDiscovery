...
namespace ExtendedControls
{
    public class TextBoxBorder : TextBox
    {
        ...
        public int BorderOffset { get; set; } = 3;
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == Win32Constants.WM.PAINT && !BorderColor.IsFullyTransparent())       // Stupid control does not have an OnPaint
            {
                using (Graphics g = Parent.CreateGraphics())
                {
                    ...
                    using (GraphicsPath g1 = RectCutCorners(clientborder.X + 1, clientborder.Y + 1, clientborder.Width - 2, clientborder.Height - 1, 1, 1))
                    using (Pen pc1 = new Pen(color1, 1.0F))
                        g.DrawPath(pc1, g1);

                    using (GraphicsPath g2 = RectCutCorners(clientborder.X, clientborder.Y, clientborder.Width, clientborder.Height - 1, 2, 2))
