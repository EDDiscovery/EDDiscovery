...
{
    public class PictureBoxHotspot : PictureBox
    {
        ...
        #endregion // public class ImageElement
        ...
        public delegate void OnElementEventHandler(object sender, MouseEventArgs eventargs, ImageElement i, object tag);

        public event OnElementEventHandler ClickElement;    // ✓
        public event OnElementEventHandler EnterElement;    // ✓
        public event OnElementEventHandler LeaveElement;    // ✓
        ...
        public void Render(bool resizecontrol = true, Size? minsize = null, Size? margin = null)
        {
            ...
            if (max.Width > 0 && max.Height > 0) // will be zero if no elements
            {
                ...
                if (resizecontrol)
                    Size = new Size(max.Width, max.Height);
            }
            else
        ...
        private ToolTip hovertip = null;                                // ✓ (ClearHoverTip)

        private Point hoverpos = Point.Empty;
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            Focus();
            ClearHoverTip();
            OnClickElement(e, elementin, elementin?.tag);   // null if no element clicked
        }
        ...
        protected override void OnMouseMove(MouseEventArgs eventargs)
        {
            ...
            {
                OnLeaveElement(eventargs, elementin, elementin?.tag);
                LeaveCurrentElement();
            }
            ...
            if (elementin == null)      // is in?
            {
                foreach (ImageElement i in elements)
                {
                    if (i.pos.Contains(eventargs.Location))
                    {
                        ...
                        OnEnterElement(eventargs, elementin, elementin.tag);
                    }
                }
            }
        ...
        protected virtual void OnEnterElement(MouseEventArgs args, ImageElement i, object tag)
        {
            EnterElement?.Invoke(this, args, i, tag);
        }

        protected virtual void OnClickElement(MouseEventArgs args, ImageElement i, object tag)
        {
            ClickElement?.Invoke(this, args, i, tag);
        }

        protected virtual void OnLeaveElement(MouseEventArgs args, ImageElement i, object tag)
        {
            LeaveElement?.Invoke(this, args, i, tag);
        }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            ClickElement = EnterElement = LeaveElement = null;
            elements = null;
            hovertimer = null;
